...
from .helpers import get_poster_url
...
def category_detail(request, category_id):
    ...
    # 排序
    if sort == 'rating':
        films = films.annotate(avg_rating=Avg('reviews__rating')).order_by(
            f'-avg_rating' if order == 'desc' else 'avg_rating'
        )
...
def add_review(request, film_id):
    ...
    if request.method == 'POST':
        ...
        if existing_review:
            initial_data = {'rating': existing_review.rating, 'comment': existing_review.content}
...
def get_chart_data(request):
    ...
    category_data = Category.objects.annotate(
        film_count=Count('films')
    ).values('name', 'film_count')
    
    rating_data = Film.objects.values('rating').annotate(
        count=Count('id')
    ).order_by('rating')
    
    year_data = Film.objects.filter(release_date__isnull=False).annotate(
    ...
    return JsonResponse({
        ...
        'ratings': list(rating_data),
        'years': list(year_data),
