...
class Film(models.Model):
    ...
    # 保存海报的相对路径（相对于static目录）
    poster_path = models.CharField(max_length=255, blank=True, null=True, verbose_name='海报路径')
    ...
    def poster_url(self):
        """获取海报URL"""
        # 先检查是否有本地海报文件
        if self.poster_path and os.path.exists(os.path.join(settings.STATIC_ROOT, self.poster_path.lstrip('/'))):
        ...
        try:
            ...
            tmdb_api = TMDBApi()
            return tmdb_api.get_or_download_poster(self)
...
class UserProfile(models.Model):
    ...
    favorite_categories = models.ManyToManyField(Category, blank=True, related_name='favorite_users',
                                                 verbose_name='喜欢的分类')
    created_at = models.DateTimeField(auto_now_add=True, verbose_name='创建时间')
    updated_at = models.DateTimeField(auto_now=True, verbose_name='更新时间')
        ...
        if self.avatar and hasattr(self.avatar, 'url'):
            try:
                if os.path.exists(self.avatar.path):
                    ...
                    return "https://via.placeholder.com/100x100?text=Missing+Avatar"
            except:
                return "https://via.placeholder.com/100x100?text=Error"
        else:
            return "https://via.placeholder.com/100x100?text=No+Avatar"